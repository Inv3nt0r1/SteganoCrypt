...
# hide payload file into LSB bits of an image
def hide(imgFile, payload, password):
	# Process source image
	try:
		img = Image.open(imgFile)
	except IOError:
		print("[-] Image does not exist. Recheck the image name. ")
		sys.exit()
	...
	try:
		...
		print("[-] Data file does not exist. Recheck the file name. ")
		sys.exit()
	data = f.read()
	f.close()
	...
	if (payload_size > max_size - 4):
		print "[-] Cannot hide. File too large\n\n"
		sys.exit()
	...
	steg_img.save(imgFile[:-4] + "-stego.png", "PNG")
...
def extract(in_file, out_file, password):
	...
	print "[+] Written extracted data to %s.\n\n" % out_file
...
def usage(progName):
	print(" Guided mode: use 'python %s'" % progName)
...
if __name__ == "__main__":
	if(len(sys.argv)==1):
		...
		print(" [3] Analyse steganographic image")
		print(" [4] Exit")
		...
		if choice == '1':
			while True:
				...
				break
			while True:
				msgname = raw_input("Enter filename of data file to hide: ")
				if(os.path.exists(msgname)==False):
					print("[-] Data file does not exist. Recheck the file name. ")
					continue
				break
			password = raw_input("Enter password for encryption: ")
			hide(imagename,msgname,password)
		elif choice == '2':
			while True:
				imagename = raw_input("Enter steganographic image name: ")
				if(os.path.exists(imagename)==False):
					print("[-] Image does not exist. Recheck the image name. ")
					continue
				break
			...
			extract(imagename,outfile,password)
		elif choice == '3':
			while True:
				imagename = raw_input("Enter steganographic image name to analyse: ")
				if(os.path.exists(imagename)==False):
					print("[-] Image does not exist. Recheck the image name. ")
					continue
				break
		...
		print("[-] Invalid Usage detected. See below: ")
		usage(sys.argv[0])